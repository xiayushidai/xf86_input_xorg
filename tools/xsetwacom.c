...
static void list_one_device(Display *dpy, XDeviceInfo *info)
{
	...
	if (!wacom_prop)
		wacom_prop = XInternAtom(dpy, "Wacom Tool Type", True); //返回wacom的标识符。

	dev = XOpenDevice(dpy, info->id); //通过设备id获取设备。
	if (!dev)
		return;

	atoms = XListDeviceProperties(dpy, dev, &natoms); //获取设备的属性
	if (natoms)
	{
		int j;
		for (j = 0; j < natoms; j++)
		...
		if (j < natoms)
		{
			...
			if (nitems)
			{
				type_name = XGetAtomName(dpy, *(Atom*)data);  //将设备信息打印到终端上。
...
static void list_devices(Display *dpy)
{
	...
	wacom_prop = XInternAtom(dpy, "Wacom Tool Type", True); //返回相关连的院子标识符，如果返回none，表示没有wacom设备，直接return。
	if (wacom_prop == None)
		return;
	...
	{
		if (info[i].use == IsXPointer || info[i].use == IsXKeyboard) //如果设备是鼠标和键盘，跳过，只针对wacom设备。
			continue;
...
static void list_param(Display *dpy)
{
	...
	while(param->name)
	{
		printf("%-16s - %16s\n", param->name, param->desc); //列出支持的参数
		param++;
	}
}

static void list_mod(Display *dpy) //显示所有修饰符
{
...
static void list(Display *dpy, int argc, char **argv)
{
	...
	if (argc == 0)
		list_devices(dpy);
		 ...
		 strcmp(argv[0], "devices") == 0)  //显示设备
		list_devices(dpy);
	else if (strcmp(argv[0], "param") == 0 ||  //显示参数
		 strcmp(argv[0], "parameters") == 0)
		list_param(dpy);
	else if (strcmp(argv[0], "mod") == 0 ||
		 strcmp(argv[0], "modifiers") == 0) //显示修饰符
		list_mod(dpy);
	else
...
static void set_mode(Display *dpy, XDevice *dev, param_t* param, int argc, char **argv)
{
	...
	if (argc != param->arg_count) //传来的参数与定义的参数做比较。
	{
		fprintf(stderr, "'%s' requires exactly %d value(s).\n", param->name,
			param->arg_count);
		return;
	...
	if (strcasecmp(argv[0], "Relative") == 0) //不计较大小写，进行比较
		mode = Relative;
	...
	XSetDeviceMode(dpy, dev, mode); //真正的设置属性。
	XFlush(dpy);
}
...
static void set(Display *dpy, int argc, char **argv)
{
	...
	//以执行xsetwacom set 15 mode absolute为例
	TRACE("'set' requested for '%s'.\n", argv[0]);

	dev = find_device(dpy, argv[0]); //argv[0]=15
	printf("dev=%ld %d %c %c\n",dev->device_id,dev->num_classes,dev->classes->event_type_base,dev->classes->input_class);
	printf("argc==%d\n",argc);
	printf("argv[0]==%s\n",argv[0]);
	// 15 7 C
	...
	param = find_parameter(argv[1]); //argv[1]=mode
	if (!param)
	{
		...
		goto out;
	} else if (param->prop_flags & PROP_FLAG_READONLY) //如果是只读属性，对于set操作，会报错。
	{
	...
	if (param->set_func) //执行set操作。
	{
		param->set_func(dpy, dev, param, argc - 2, &argv[2]); //argc=3,3-2=1.
		goto out;
	}
...
static void get_mode(Display *dpy, XDevice *dev, param_t* param, int argc, char **argv)
{
	...
	info = XListInputDevices(dpy, &ndevices); //ndevices 是设备的个数
	printf("ndevices=",ndevices);

	for (i = 0; i < ndevices; i++)
	{
		d = &info[i];
		printf("info.id==%d %s %ld\n",i,d->name,d->id);
...
static void get(Display *dpy, enum printformat printformat, int argc, char **argv)
{
	...
	dev = find_device(dpy, argv[0]); //通过name获取设备信息。
	if (!dev)
	{
		fprintf(stderr, "Cannot find device '%s'.\n", argv[0]);
		return;
	...
	param = find_parameter(argv[1]); //获取设备的某个参数信息。
	if (!param)
	{
		if (is_deprecated_parameter(argv[1]))
			return;
		fprintf(stderr, "Unknown parameter name '%s'.\n", argv[1]);
		goto out;
	} else if (param->prop_flags & PROP_FLAG_WRITEONLY)  //没有read属性只有写属性
	{
...
static bool check_for_wayland(Display *dpy)
{
	...
	info = XListInputDevices(dpy, &ndevices); //通过返回的设备名称，判断是x11还是wayland环境。
	for (i = 0; i < ndevices; i++) {
...
int main (int argc, char **argv)
{
	...
	TRACE("Display is '%s'.\n", display);
	printf("display==%s\n",display);
	dpy = XOpenDisplay(display); //xlib函数，获得display的句柄
	if (!dpy)
	{
	...
	if (!do_list && !do_get && !do_set)
	{
		if (optind < argc)
		{
			if (strcmp(argv[optind], "list") == 0)  //list入参
			{
				do_list = 1;
				optind++;
			} else if (strcmp(argv[optind], "set") == 0)  //set入参
			{
				do_set = 1;
				optind++;
			} else if (strcmp(argv[optind], "get") == 0)  //get 属性
			{
				do_get = 1;
				optind++;
			}
