...
static int wcmAllocate(InputInfoPtr pInfo)
{
	...
	priv->nPressCtrl [0] = 0;    /* pressure curve x0 */  //压力曲线 一共四个点。
...
{
	system("echo 0 >> /home/test/jian.txt");
...
static int NewWcmPreInit(InputDriverPtr drv, InputInfoPtr pInfo, int flags)
#else
static int wcmPreInit(InputDriverPtr drv, InputInfoPtr pInfo, int flags)  //wacom设备的驱动程序入口，每当有设备加载时，都会调用它。
#endif
{
	system("echo 1 >> /home/test/jian.txt");
	WacomDevicePtr priv = NULL;
	WacomCommonPtr common = NULL;
	char		*type, *device;
	char		*oldname = NULL;
	...
	//dev/input/event4   event5
	device = xf86SetStrOption(pInfo->options, "Device", NULL);
	type = xf86SetStrOption(pInfo->options, "Type", NULL);
	xf86Msg(X_INFO, "zjg : device:%s type:%s\n",device,type);
	/*
	   Init process:
	...
	if (!wcmAllocate(pInfo))	//对Pinfo的private和其他进行初始化。
		goto SetupProc_fail;

	if (!device && !(device = wcmEventAutoDevProbe(pInfo))) //自动在/dev/input下面检测wacom设备。
		goto SetupProc_fail;

	priv = (WacomDevicePtr) pInfo->private;
	priv->common->device_path = device;
	priv->name = pInfo->name;
	priv->debugLevel = xf86SetIntOption(pInfo->options, //调试等级。
					    "DebugLevel", priv->debugLevel);

	/* check if the same device file has been added already */
	if (wcmIsDuplicate(device, pInfo)) //检查是否有重复设备。
		goto SetupProc_fail;

	if (wcmOpen(pInfo) != Success)
		goto SetupProc_fail;

	/* Try to guess whether it's USB or ISDV4 */  //检查接口是usb还是isdv4，目前只有这两种，通过ioctl的第二个参数控制。
	...
	if (wcmIsHotpluggedDevice(pInfo)) //是否为热插拔设备。
		is_dependent = 1;
	...
	if (!wcmSetType(pInfo, type))  //设置设备的type，比如绝对和相对模式。
		goto SetupProc_fail;
...
static InputDriverRec WACOM =  //WACOM的驱动程序
{
	1,             /* driver version */
	"wacom",       /* driver name */
	NULL,          /* identify */
